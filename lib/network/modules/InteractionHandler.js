  ...
  onDrag(event) {
    ...
    if (selection && selection.length && this.options.dragNodes === true) {
      ...
      selection.forEach((selection) => {
        ...
        //if dragged node is a labelNode, constrain movement to within x/y: 20 of the parent node
        if (node.options.isLabelNode == true) {
          //temp easy test to see if we hit this logic
          // console.log("hit label node");
          // console.log ( Math.min(Math.max(parseInt(deltaX), -20), 20) , Math.min(Math.max(parseInt(deltaY), -20), 20));
          deltaX = Math.min(Math.max(parseInt(deltaX), -20), 20);
          deltaY = Math.min(Math.max(parseInt(deltaY), -20), 20);
        }
