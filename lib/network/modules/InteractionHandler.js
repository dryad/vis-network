      ...
      selection.forEach((selection) => {
        ...
        if (node !== undefined) {
          ...
          if (node.options.isLabelNode == true) {
            // get the node that this is the label for
            const labelOfNode = this.body.nodes[node.options.labelOfNode];

            //deltaX should be the difference between the cursor and the labelOfNode
            deltaX =
              this.canvas._XconvertDOMtoCanvas(pointer.x) - labelOfNode.x;
            deltaY =
              this.canvas._YconvertDOMtoCanvas(pointer.y) - labelOfNode.y;

            // constrain movement of labelNode to within x/y of parent node
