...
class InteractionHandler {
  ...
  onDrag(event) {
    ...
    // console.log('onDrag selection', selection);
    if (
      selection &&
      selection.length &&
      this.options.dragNodes === true &&
      pointer !== undefined &&
      ...
      selection.forEach((selection) => {
        ...
        if (node !== undefined) {
          //if dragged node is a labelNode, constrain movement to within x/y: 40 of the parent node
          if (node.options.isLabelNode == true) {
            deltaX = Math.min(Math.max(parseInt(deltaX), -40), 40);
            deltaY = Math.min(Math.max(parseInt(deltaY), -40), 40);

            //get parent node in case we need to use its position in further updates
            // let parentNode = this.body.nodes[node.options.labelOfNode];
            ...
            // handle movement of non-labelNodes
            // only move the node if it was not fixed initially
            if (selection.xFixed === false) {
              node.x = this.canvas._XconvertDOMtoCanvas(
                this.canvas._XconvertCanvasToDOM(selection.x) + deltaX // vis mod - now using moveAnchor.x instead of selection.x
              );
            }
            // only move the node if it was not fixed initially
            if (selection.yFixed === false) {
              node.y = this.canvas._YconvertDOMtoCanvas(
                this.canvas._YconvertCanvasToDOM(selection.y) + deltaY // vis mod - now using moveAnchor.y instead of selection.y
              );
            }
          }
        }
      ...
      if (event.srcEvent.shiftKey) {
        ...
        if (this.drag.pointer === undefined) {
          // console.log('if the drag was not started properly because the click started outside the network div, start it now.')
      ...
      if (this.options.dragView === true && !event.srcEvent.shiftKey) {
        ...
        // if the drag was not started properly because the click started outside the network div, start it now.
        if (this.drag.pointer === undefined) {
          // console.log('Instance 2: if the drag was not started properly because the click started outside the network div, start it now.')
          this.onDragStart(event);
          return;
        }
