...
class ManipulationSystem {
  ...
  _controlNodeTouch(event) {
    ...
    // console.log('_controlNodeTouch', this.lastTouch);
  ...
  _controlNodeDragStart() {
    ...
    // console.log('pointerObj', pointerObj);
  ...
  _handleConnect(event) {
    ...
    if (new Date().valueOf() - this.touchTime > 100) {
      ...
      // console.log("handleConnect on node", node);
      if (node !== undefined) {
        if (node.isCluster === true) {
          alert(
            this.options.locales[this.options.locale]["createEdgeError"] ||
          ...
          if (node.isLabelNode === true) {
            ...
            // this.selectionHandler.generateClickEvent("dragEnd", event); // cannot use dragEnd while things are moving, it will create undo steps.
  ...
  _dragControlNode(event) {
    ...
    let node = undefined;
    for (let i = overlappingNodeIds.length - 1; i >= 0; i--) {
      // console.log('overlapping node1', overlappingNodeIds[i]);
      // console.log('isLabelNode1', this.body.nodes[overlappingNodeIds[i]].isLabelNode);
  ...
  _dragStartEdge(event) {
    // console.log("dragStartEdge", event);
    // if (this.lastTouch) {
    // console.log('this.lastTouch', this.lastTouch);
    const pointer = this.lastTouch;
    this.selectionHandler.generateClickEvent(
      "dragStart",
      event,
