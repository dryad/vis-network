...
class ManipulationSystem {
  ...
  getSelectedNodes() {
    return this.selectionHandler.getSelectedNodeIds();
  }
      ...
      if (deleteFunction.length === 2) {
        deleteFunction(data, (finalizedData) => {
          if (
            ...
            // find all edges that contain any of the node ids in selectedNodes, and delete them
            const edgesDataSet = this.body.data.edges.getDataSet();

            for (const edge of edgesDataSet.get()) {
              if (
                selectedNodes.indexOf(edge.from) !== -1 ||
                selectedNodes.indexOf(edge.to) !== -1 ||
                selectedNodes.indexOf(edge.eventual) !== -1
              ) {
                finalizedData.edges.push(edge.id);
              }
            }
      ...
      if (node !== undefined) {
        if (node.isCluster === true) {
          ...
          if (node.isLabelNode === true) {
            // if the add edge tool is clicked in a label node, do nothing, but also set selection to a blank list, to solve a bug where it was moving the last select object.
            this.interactionHandler.drag.selection = [];
            return;
          }
  ...
  _dragControlNode(event) {
    ...
    for (let i = overlappingNodeIds.length - 1; i >= 0; i--) {
      ...
      if (this.temporaryIds.nodes.indexOf(overlappingNodeIds[i]) === -1) {
        // if the node is NOT a labelNode, accept the node.
        if (this.body.nodes[overlappingNodeIds[i]].isLabelNode !== true) {
          node = this.body.nodes[overlappingNodeIds[i]];
          break;
        }
      }
   ...
   * @param {Node.id} sourceNodeId
   * @param {Node.id} targetNodeId
   * @param eventualNodeId
   * @private
   */
  _performEditEdge(sourceNodeId, targetNodeId, eventualNodeId) {
    const defaultData = {
      id: this.edgeBeingEditedId,
      from: sourceNodeId,
      to: targetNodeId,
      eventual: eventualNodeId,
