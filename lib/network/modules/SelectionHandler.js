...
class SelectionHandler {
  ...
  async hoverObject(event, pointer) {
    ...
    // console.log('hoverObject', object);
    let hoverChanged = false;

    if (object === undefined) {
      //experimental
      // if the mouse is not on top of an object, we remove all hover highlights
      if (this.hoverObj.nodes.length > 0 || this.hoverObj.edges.length > 0) {
        for (let i = 0; i < this.hoverObj.nodes.length; i++) {
          this.emitBlurEvent(event, pointer, this.hoverObj.nodes[i]);
        }
    ...
    if (object !== undefined) {
      ...
      if (hoverChanged || newOnlyHoveredEdge || newOnlyHoveredNode) {
        // console.log('calling emitHoverEvent', object.options.id);
        // console.log('nodes objects at emitHoverEvent', this.body.nodes);
        // Search algorithm goes here. When newly hovered object

        hoverChanged = this.emitHoverEvent(event, pointer, object);
        // hoverChanged = true; // to request redraw below
      }

      if (newOnlyHoveredNode) {
        // original test - highlight all nodes by id.
        // console.log('newOnlyHoveredNode is true so starting loop');
        // for (const nodeId in this.body.nodes) {
        //   const node = this.body.nodes[nodeId];
        //   if (node.isLabelNode !== true) {
        //     // console.log('hovering extra node: ', node.options.id);
        //     this.emitHoverEvent(event, pointer, node);
        //   }
        // }

        // depth first algorithm goes here. Runs when a new single node is hovered.
        ...
        const find_subordinate_nodes = (node_id) => {
          if (this.body.nodes[node_id].level !== 0) {
