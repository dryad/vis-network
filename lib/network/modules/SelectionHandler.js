...
class SelectionHandler {
  ...
  selectObject(obj, highlightEdges = this.options.selectConnectedEdges) {
    if (obj !== undefined) {
      ...
      this.body.emitter.emit("selectedNodes", selectedNodes); // we give the list of selected nodes to Utensil with the selectedNodes event
      return true;
    }
    return false;
  }
  ...
  /**
   * Perform hover in response to a setHover call
   * Updates the current hovered set
   *
   * @param {{nodes: Array.<string>}} hover | object with the x and y screen coordinates of the mouse
   */
  async setHover(hover) {
    // console.log('setHover: ', hover);
    if (hover.node == null) {
      for (const node in this.body.nodes) {
        node.hover = false;
        delete this.hoverObj.nodes[node.id];
        // console.log('node', node.id, 'hover', node.hover);
      }
      // console.log('this.hoverObj.nodes', this.hoverObj.nodes);
      this.body.emitter.emit("hoveredNodes", []);
      this.body.emitter.emit("_requestRedraw");
      return;
    }
    if (!hover || !hover.node) {
      throw new TypeError("Hover must be an object with a node id as a string");
    }

    let hoverChanged = false;

    const object = this.body.nodes[hover.node];
    if (object !== undefined) {
      // const hoveredEdgesCount = Object.keys(this.hoverObj.edges).length;
      // const hoveredNodesCount = Object.keys(this.hoverObj.nodes).length;
      // const newOnlyHoveredEdge =
      ...
      if (object instanceof Node) {
        ...
        // re-use the same find_subordinate_nodes from the hover event, and remove this duplicate code
        const find_subordinate_nodes = (node_id) => {
          if (
            this.body.nodes[node_id] &&
            this.body.nodes[node_id].level !== 0
