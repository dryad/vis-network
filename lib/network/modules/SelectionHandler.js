  ...
  async hoverObject(event, pointer) {
    ...
    if (object === undefined) {
      ...
      if (this.hoverObj.nodes.length > 0 || this.hoverObj.edges.length > 0) {
        ...
        // move this so it only fires once on unselect, it was firing every frame the mouse moves over nothing
    ...
    if (object !== undefined) {
      ...
      if (
        object instanceof Node &&
        (window.isUserDragging == undefined || window.isUserDragging == false)
      ) {
