...
class Edge {
  ...
  constructor(options, body, imagelist, globalOptions, defaultOptions) {
    ...
    this.eventual = undefined; // a node, the third in the connection. added by mike.
  ...
  setOptions(options) {
    ...
    let affectsLayout =
      ...
      (typeof options.eventual !== "undefined" &&
        this.options.eventual !== options.evenual) ||
      (typeof options.directed !== undefined &&
        this.options.directed !== options.directed);
    ...
    if (options.eventual !== undefined) {
      this.eventualId = options.eventual;
    }
  ...
  connect() {
    ...
    this.eventual = this.body.nodes[this.eventualId] || undefined;
    this.connected = this.from !== undefined && this.to !== undefined; // &&
    //this.eventual !== undefined;
    //checking for eventual here is causing problems with 2-node edges (unary operator)

    if (this.connected === true) {
      this.from.attachEdge(this);
      this.to.attachEdge(this);
      if (this.eventual) {
        this.eventual.attachEdge(this);
      }
    } else {
      if (this.from) {
  ...
  drawArrows(ctx) {
    ...
    // console.log('this.edgeType.from', this.edgeType.from);
    // console.log('this.edgeType.to', this.edgeType.to);
    // console.log('this.edgeType.eventual', this.edgeType.eventual);
    // get the via node from the edge type
    const viaNode = this.edgeType.getViaNode();
    const arrowData = {};
    ...
    this.edgeType.eventualPoint = this.edgeType.eventual;
    ...
    if (values.toArrow) {
      ...
      if (values.arrowStrikethrough === false) {
        this.edgeType.toPoint = arrowData.to.core; // original
        //this.edgeType.fromPoint = arrow.from.core; // added to make it aware of fromPoint
        //this.edgeType.toPoint = arrow.to.core;
        //this.edgeType.eventualPoint = arrow.eventual.core;
        //this.edgeType.eventualPoint = arrowData.to.core; // this almost works right if "middle" node is in the right spot..
      }
...
*/
  ...
  endPointsValid() {
    return (
      ...
      //this.body.nodes[this.eventualId] !== undefined
      //checking for eventualId here is causing issues with 2-node edges (unary
      //this function will have an effect on vis-network 'clustering'
    );
  }
}
