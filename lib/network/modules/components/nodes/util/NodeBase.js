...
class NodeBase {
  ...
  initContextForDraw(ctx, values) {
    ...
    // if the node is selected in the graph

    // if (window.selectedNodes) {
    //   console.log('window.selectedNodes', window.selectedNodes);
    //   console.log('window.selectedNodes.indexOf(this.options.id)', window.selectedNodes.indexOf(this.options.id));
    // }

    // console.log('this.options.id', this.options.id);
    // new method, works when window.selectedNodes contains the node id
    if (
      window.hoveredNodes &&
      window.hoveredNodes.indexOf(this.options.id) !== -1
    ) {
      ctx.shadowBlur = 15;
    ...
    // old method
    if (values.borderWidth >= 5) {
      // using values.borderWidth to detect selection without being affected by scale.
      // glow effect - if node is selected, borderWidth will be 2. Quick way to detect selection and apply the glow effect.
      ctx.shadowBlur = 15;
      ctx.shadowColor = "red";
    }
  ...
  performStroke(ctx, values) {
    ...
    if (borderWidth > 0) {
      ...
      //force border to be drawn in the original color, but with opacity of 1.
      ctx.strokeStyle = ctx.strokeStyle.replace(/[\d.]+\)$/g, "1)");
    ...
    ctx.restore();
    this.performFillNoShadow(ctx, values); // fill the node again, without a shadow, leaving just the aura.
  }
  ...
  performFillNoShadow(ctx, values) {
    //used to draw the inside of the node, overwriting the shadow, leaving just the aura.
    ctx.save();
    ctx.fillStyle = values.color;
    // draw shadow if enabled
    // this.enableShadow(ctx, values);
