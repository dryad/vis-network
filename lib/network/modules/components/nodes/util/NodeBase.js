...
class NodeBase {
   ...
   * @param {boolean} hover
   */
  initContextForDraw(ctx, values, selected, hover) {
    const borderWidth = values.borderWidth / this.body.view.scale;
    // console.log('drawing node', this.options.id);
    if (selected) {
      // red aura for selected node
      ctx.shadowBlur = 15;
      ctx.shadowColor = "red";
    } else if (hover) {
      // console.log('draw hover on node', this.options.id);
      // blue aura for hovered node
      ctx.shadowBlur = 15;
