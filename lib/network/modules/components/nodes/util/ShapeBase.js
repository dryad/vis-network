...
class ShapeBase extends NodeBase {
  ...
  _drawShape(ctx, shape, sizeMultiplier, x, y, selected, hover, values) {
    ...
    this.initContextForDraw(ctx, values, selected, hover);
    // if (this.options.isLabelNode !== true) {
    //only draw the shape of non-labelNodes
    getShape(shape)(ctx, x, y, values.size);
    // }

    // even though labelNode shapes are not drawn, allowing fill on labelNodes was filling the original node with the wrong color.
    // if (this.options.isLabelNode !== true) {
    ...
    return {
      drawExternalLabel: () => {
        if (this.options.label !== undefined && this.options.isLabelNode) {
